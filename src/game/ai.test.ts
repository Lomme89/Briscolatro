...
describe('opponent AI', () => {
  it('always returns a card that belongs to its hand', () => {
    ...
    const lead = chooseOpponentLead(hand, { briscolaSuit: 'denari', profile: { ...NEUTRAL_PROFILE, noise: 0 } });
  ...
  it('uses canonical resolver to take a valuable trick when possible', () => {
    ...
    const chosen = chooseOpponentFollow(hand, playerLead, { briscolaSuit: 'denari', profile: { ...NEUTRAL_PROFILE, noise: 0 } });
  ...
  it('returns null for an empty hand instead of inventing a card', () => {
    expect(chooseOpponentLead([], { briscolaSuit: 'denari', profile: { ...NEUTRAL_PROFILE, noise: 0 } })).toBeNull();
    expect(chooseOpponentFollow([], c('coppe', 2), { briscolaSuit: 'denari', profile: { ...NEUTRAL_PROFILE, noise: 0 } })).toBeNull();
  });
});
